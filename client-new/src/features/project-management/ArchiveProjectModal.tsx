...
import { useArchiveWorkspace } from '../../entities/workspace/useWorkspaceQuery';
...
export function ArchiveProjectModal() {
  ...
  const isVisible = activeModal === 'modal-archive-project';
  ...
  const handleArchive = async () => {
    if (!targetId) return;
    try {
      await archiveMutation.mutateAsync(targetId);
      showToast(t('archiveProject.archived'));
      closeModal();
    } catch (_e) {
      showToast(t('archiveProject.error'));
    }
  };
    ...
    <div className="fixed inset-0 bg-gray-900/40 dark:bg-black/60 backdrop-blur-sm z-[70] flex items-center justify-center fade-in">
      <div className="bg-white dark:bg-graphite-card border border-gray-200 dark:border-graphite-border w-full max-w-md rounded-2xl shadow-2xl p-6 transform transition-all modal-content pop-in relative">
        ...
        <div className="flex items-center gap-4 mb-4">
          <div className="w-12 h-12 rounded-full bg-primary-100 dark:bg-primary-900/20 flex items-center justify-center text-primary-600 shrink-0">
            <Archive className="w-6 h-6" />
          </div>
          <div>
            <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">{t('archiveProject.title')}</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400">{t('archiveProject.description')}</p>
          </div>
        </div>
        ...
        <div className="flex justify-center gap-3 mt-6">
          ...
          <button 
            ...
            className="px-5 py-2.5 text-sm font-semibold bg-primary-600 hover:bg-primary-700 text-white rounded-xl transition-colors shadow-sm w-full"
          >
            {archiveMutation.isPending ? t('archiveProject.archiving') : t('common.archive')}
          </button>
        </div>
      </div>
    </div>
